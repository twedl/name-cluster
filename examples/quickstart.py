...
def threshold_sweep() -> None:
    ...
    for t in (0.50, 0.70, 0.85, 0.95, 0.99):
        ...
        print(
            f"  {t:>9.2f}  {n_clusters:>10}  {m['adjusted_rand']:.2f}   {m['f1_pairs']:.2f}"
        )
...
def real_data_demo() -> None:
    _section("6. Real-data demo (cached GLEIF names if downloaded)")
    cache_root = (
        Path(os.environ.get("XDG_CACHE_HOME", Path.home() / ".cache")) / "name_cluster"
    )
    gleif = sorted((cache_root / "gleif" / "parquet").glob("*.parquet"))
    if not gleif:
        print("  no GLEIF cache found; skip")
        print("  to populate: uv run scripts/download_corpora.py gleif")
        return

    df = pl.read_parquet(gleif[0])
    print(f"  loaded {df.height:,} GLEIF entity-name rows from {gleif[0].name}")

    # Sample down so the demo runs in a few seconds
    sample = df.sample(n=min(10000, df.height), seed=0)
    result = nc.cluster(sample, name_col="legal_name")
    ...
    print(
        f"  clustered {sample.height:,} names -> {n_clusters} clusters "
    ...
    for row in sizes.iter_rows(named=True):
        ...
        print(
            f"    [{row['cluster_id']:>4}] {row['canonical_name']!r}  "
